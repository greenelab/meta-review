...
However, sometimes external resources provide Manubot with invalid CSL Data, which can crash downstream citation processors, such as [pandoc-citeproc](http://hackage.haskell.org/package/pandoc-citeproc).
...
If the user does not specify a CSL style, the utility defaults to using the Manubot style, which focuses on information relevant for modern scholarly workflows and omits the rest.
For example, Manubot's style places the title first, specifies publication date with day precision when possible, and uses numeric-style citations. [@url:https://blog.dhimmel.com/citation-styles/].
