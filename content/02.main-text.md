...
However, tools must still be built whose workflows encourage openness rather than stifle it.
...
For example, open access publishing provides an economic model that encourages maximal dissemination and reuse of scholarly articles [@doi:10.12688/f1000research.8460.3].
...
We developed Manubot with the goal of adapting the the open source software workflow for scholarly writing.
As such, using Manubot requires some technical expertise, but the benefits are manyfold.
...
Our hope that these attributes help integrate Manubot with an ecosystem of other community-driven tools to make science as open and collaborative as possible.
