...
There are many existing collaborative writing platforms [@doi:10.1038/514127a].
...
Traditionally, LaTeX has been used for these needs, since documents are written in plain text and the system is open source and customizable.
Rendering LaTeX documents requires specialized software, but webapps like [Overleaf](https://www.overleaf.com/) now enable collaborative authoring of LaTeX documents via a webapp.
Nonetheless, LaTeX-based systems are limited in that PDF is the only fully supported output format.
Alternatively, [Authorea](https://www.authorea.com/) is a collaborative writing webapp whose primary output format is HTML, but allows authors to write sections in Markdown, a limited subset of LaTeX, or their WYSIWIG HTML editor.
Collaborative writing tools ideally offer version control, multiple permission levels, or other functionality to support multi-author document editing.
...
For example, the [Kipoi](https://kipoi.org/) model zoo for genomics [@doi:10.1101/375345] uses Manubot's Python interface to retrieve model authors from persistent identifiers.
...
In the video [@doi:10.6084/m9.figshare.7946192.v1], a contributor creates a [pull request](https://github.com/manubot/try-manubot/pull/2) to add a sentence to the try-manubot manuscript.
