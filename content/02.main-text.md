...
Specifically, a `content` directory contains one-or-more Markdown files that define the body of the manuscript as well as a YAML-formatted file specifying manuscript metadata (e.g. the title, authors, keywords, and language).
...
Many of these operations are delegated to the `manubot` [Python package](https://github.com/manubot/manubot) or other dependencies such as Pandoc and Travis CI.
Manubot pieces together many existing standards and technologies to enable encapsulating a manuscript in a repository where the generation of outputs is automated.
...
For this functionality, Manubot relies on a the [`pandoc-xnos`](https://github.com/tomduck/pandoc-xnos) suite of Pandoc filters.
A list of formatting options officially supported by Manubot, at the time of writing, is viewable as [raw markdown](https://gitlab.com/manubot/rootstock/blob/fea9eb80af4ca9d5a116f6c28a6b740272e3c0b9/content/02.delete-me.md#L) and the corresponding [rendered HTML](https://manubot.github.io/rootstock/v/fea9eb80af4ca9d5a116f6c28a6b740272e3c0b9/).
...
An example repository at <https://github.com/manubot/rootstock> demonstrates Manubot's features and serves as a template for users to write their own manuscripts with Manubot.
...
The setup process is complex, but most only be performed once per manuscript.
Upgrading manuscripts is also possible by pulling the latest commits from Rootstock, but sometimes involves resolving Git conflicts.
...
In addition, `manubot cite` command line utility takes a list of citations and returns either a rendered bibliography or CSL Data Items (i.e. JSON-formatted reference metadata).
