...
First, open access publishing provided an economic model that encourages maximal dissemination and reuse of scholarly articles.
Pressure from funders, libraries, self-archiving, and piracy are set to cement open access as the primary model for publishing the scientific record [@doi:10.7554/eLife.16800].
However, open science extends beyond just open access to journal articles.
Open licensing solves legal barriers to content reuse, enabling any type of scholarly output to become part of the commons.
...
The push for increased reproducibility and the opportunity to reuse data and code for new investigations has begot a movement to make all research outputs public, unless there are bonafide privacy or security concerns.
