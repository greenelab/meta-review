...
Manubot ensures its CSL Items validate according to the official JSON schema by removing problematic fields, which are prone to crashing downstream citation processors, such as [pandoc-citeproc](http://hackage.haskell.org/package/pandoc-citeproc) in Manubot's case.
...
Thousands of journals have [predefined styles](http://editor.citationstyles.org/searchByName/) [@tag:techblog-csl].
...
This utility produces CSL Items (i.e. JSON-formatted reference metadata) for a list of citations.
For example, `manubot cite doi:10.1098/rsif.2017.0387`, returns the CSL Item for the Deep Review.
In addition, the utility can render the references into a formatted bibliography.
For example, the following command outputs a Markdown reference list according to the bibliographic style of _PeerJ_:
...
If the user does not specify a CSL style, the program defaults to using the Manubot style, which focuses on information relevant for modern scholarly workflows and omits the rest.
The first line provides the title of the work in bold.
The second line lists the full names of authors.
The third line provides the journal or publisher name in italics, the date of publication (in YYYY-MM-DD format if possible), and the URL.
The final line provides CSL-supported standard identifiers, such as the DOI, PMID, and PMCID.
Finally, numeric style rather than author style is used to render in-text citations [@url:https://blog.dhimmel.com/citation-styles/].
