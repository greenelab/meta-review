...
We are also [exploring](https://github.com/manubot/manubot/pull/99 "Create a pandoc-manubot-cite filter for pandoc") whether we can implement citation-by-identifier as a Pandoc filter.
This would allow Pandoc users to cite persistent identifiers as part of their existing Pandoc workflows, without requiring them to adopt other aspects of Manubot.
Since Pandoc already has a large user base, who convert to and from from a broad range of formats for a broad range of applications, a cite-by-identifier filter could help popularize the practice at an influential scale.
...
In addition, to simpler usage, we're also looking into simplified setup.
...
While we provide detailed instructions, users often struggle replicating the long list of setup commands.
...
In addition to simplifying setup, such migrations may also present the oppertunity to decrease dependency on propreitary services and address other Manubot shortcomings, such as the current inability to view rendered manuscripts produced by pull request builds.
...
We are happy to provide users _pro bono_ assistance to upgrade manuscripts.
...
For example, a journal may decide build their publishing workflow around Manubot, such that submissions would consist of a Manubot repository.
...
Likewise, Manubot may excell at enabling collaborative translation of existing manuscripts into other languages.
Another niche application could become collaborative development of online lessons, documentation, or tutorials.
