...
These plugins add special interactive features that enhance the user experience of viewing and reading manuscripts (@fig:plugins).
...
![
Screenshots of the various Manubot plugins, illustrating their functionality and usefulness.
](images/plugins.png "Manubot Plugins"){#fig:plugins}

The Manubot "front-end" (layout, look, controls, behavior, etc.) is designed to be in line with current best practices and user expectations of the modern web.
In addition, the plugins utilize standard technology built in to (almost) all major web browsers, allowing them to be relatively lightweight, modular, and easy to configure.
